<template>
  <div>
    <div>
      <mt-header title="学前端 到学问" fixed>
        <div slot="right" class="show">
          <router-link to="/register">免费注册</router-link>
          <router-link to="login">登录</router-link>
        </div>
      </mt-header>

      <mt-navbar v-model="test" class="navbar">
        <mt-tab-item
          v-for="(categorie,index) of categories"
          :key="index"
          :id="categorie.id"
        >{{categorie.category_name}}</mt-tab-item>
      </mt-navbar>

      <mt-tab-container v-model="test" class="content">
        <mt-tab-container-item :id="categorie.id" v-for="(categorie,index) of categories" :key="index"
        >
         <div class="info-item" v-for="(article,index) of articles" :key="index">
          <div class="info-item-head">
            <a href="#">{{article.subject}}</a>
          </div>
          <div class="info-item-content">
            <div class="info-item-img" v-if="article.image!=null">
              <img v-lazy="article.image">
            </div>
            <div class="info-item-des">{{article.description}}</div>
          </div>
        </div>
        </mt-tab-container-item>
      </mt-tab-container>
 
      <mt-tabbar v-model="selectTab" fixed>
        <mt-tab-item id="home">
          <img v-if="selectTab=='home'" src="../assets/images/home_enable.png" slot="icon">
          <img v-else src="../assets/images/home_disable.png" slot="icon">
        </mt-tab-item>
        <mt-tab-item id="me">
          <img v-if="selectTab=='me'" src="../assets/images/me_enable.png" slot="icon">
          <img v-else src="../assets/images/me_disable.png" slot="icon">
        </mt-tab-item>
        <mt-tab-item id="cart">
          <img v-if="selectTab=='cart'" src="../assets/images/cart_enable.png" slot="icon">
          <img v-else src="../assets/images/cart_disable.png" slot="icon">
        </mt-tab-item>
      </mt-tabbar>
    </div>
  </div>
</template>



<script>
export default {
  data() {
    return {
      test: 1,
      selectTab: "home",

      categories: [],
      articles: [],
      page: 1
    };
  },

  methods: {
      loadData(){
          this.axios.get('/articles01?cid='+this.test+'&page='+this.page).then((res)=>{
              console.log(res.data);
          })
      }
  },

  watch: {
    test() {
      this.page++;
      this.axios
        .get("/articles01?cid=" + this.test + "&page=" + this.page)
        .then(res => {
          //console.log(res.data);
          res.data.forEach(item => {
            if (item.image != null) {
              item.image = require("../assets/images/articles/" + item.image);
            }
            this.articles.push(item);
          });
        });
    }
  },

  mounted() {
    this.axios.get("/categories01").then(res => {
      //console.log(res.data);
      this.categories = res.data.result;
    });

    this.axios
      .get("/articles01?cid=" + this.test+ "&page=" + this.page )
      .then(res => {
        console.log(res.data);
        res.data.forEach(item => {
          if (item.image != null) {
            item.image = require("../assets/images/articles/" + item.image);
          }
          this.articles.push(item);
        });
       
      });
  }
};
</script>





<style scoped>
.show > a {
  text-decoration: none;
  color: #fff;
  padding: 3px;
}
.content {
  margin-bottom: 43px;
}
.navbar {
  margin-top: 40px;
}
.info-item {
  border-bottom: 0.5px solid #d3d3d3;
  padding: 10px;
}
.info-item-head > a {
  font-weight: bold;
  font-size: 17px;
  color: #1a1a1a;
  line-height: 22px;
  text-decoration: none;
}
.info-item-img > img {
  width: 100px;
  height: 60px;
  margin-right: 15px;
  border-radius: 5px;
}
.info-item-content {
  display: flex;
  margin-top: 5px;
}
.info-item-des {
  font-weight: 400;
  font-size: 17px;
  color: #444;
  line-height: 21px;
  height: 63px;
  overflow: hidden;
}
</style>
